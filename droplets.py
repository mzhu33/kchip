...
def find_droplets(config,image,threshold=0.53,show=1):
    ...
    # Droplet size range should be radius range of 50 um to 80 um
    hough_radii = np.arange(50.*config['image']['objective']/config['image']['bins']/config['image']['pixel_size'],80.*config['image']['objective']/config['image']['bins']/config['image']['pixel_size'])
    ...
    radii_val = int(50*config['image']['objective']/config['image']['bins']/config['image']['pixel_size'])
    accum, cx, cy, rad = hough_circle_peaks([hough_res],
                                            [radii_val],
                                            threshold=threshold,
                                            min_xdistance=int(radii_val*1.5),
                                            min_ydistance=int(radii_val*1.5))
...
#             axes.plot(circx,circy,'r')
    ...
    return pd.DataFrame(data=np.vstack((cx,cy,rad)).T,columns=['ImageX','ImageY','Radius'])
