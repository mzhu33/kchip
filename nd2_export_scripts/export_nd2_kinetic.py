...
# python export_nd2_kinetic.py --nd2file ../data/ChannelA647_7p_1X,A594_0.1p_1X,A555_1p_1X,GFP_Seq0000.nd2 --savepath ../output/nd2_export2/ --root-name kinetic --rectangle 1 --rows 4 --cols 5
...
def main(nd2file,savepath,root_name,rectangle,rows,cols):
    with ND2_Reader(nd2file) as nd2:
        ...
        if rectangle == 1:
            ...
            root_names = []
            for i in range(tp):
                sublist = [root_name+'_t'+str(i)+'_']*m
                root_names.append(sublist)
                root_names_ = [item for sublist in root_names for item in sublist]
            df['root_name'] = root_names_
            df['tile_id'] = list(range(1,1+m))*tp
